...
import { analysisPath, YOUTUBE_ANALYZER_TRANSCRIPT } from '@/tools/youtube-analyzer/lib/paths';
import { formatCompactNumber } from '@/tools/youtube-analyzer/lib/utils';

interface SavedAnalysesPanelProps {
  analyses: SavedChannelAnalysisSummary[];
  deletingId: string | null;
  error: string | null;
  onDelete: (analysisId: string, channelTitle: string) => void;
}
...
export function SavedAnalysesPanel({ analyses, deletingId, error, onDelete }: SavedAnalysesPanelProps) {
  return (
    <section className="rounded-3xl border border-white/10 bg-zinc-900/80 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold text-white">Open a snapshot, delete one, or start a new analysis above.</h2>
        </div>
          ...
          analyses.map((analysis) => {
            ...
            return (
              <article className="rounded-2xl border border-white/10 bg-zinc-950/70 p-4 transition hover:border-white/20 hover:bg-zinc-950/90" key={analysis.id}>
                <Link className="block" href={analysisPath(analysis.id)}>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h3 className="text-base font-medium text-white">{analysis.channelTitle}</h3>
                      <p className="mt-1 text-sm text-zinc-400">Saved {formatRelativeTime(analysis.analyzedAt)}</p>
                    </div>
                  ...
                  {analysis.topTakeaway ? <p className="mt-4 text-sm text-zinc-300">{analysis.topTakeaway}</p> : null}
                </Link>
                ...
                <div className="mt-4 flex justify-end">
                  <button
                    ...
                    disabled={deleting}
                    onClick={() => onDelete(analysis.id, analysis.channelTitle)}
                    type="button"
                  >
