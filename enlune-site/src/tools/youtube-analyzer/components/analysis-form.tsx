...
import type { AnalyzeChannelResponse } from '@/tools/youtube-analyzer/lib/contracts/api';
import { upsertClientAnalysis } from '@/tools/youtube-analyzer/lib/client-analysis-store';
import { analysisPath } from '@/tools/youtube-analyzer/lib/paths';
...
export function AnalysisForm({ title, description, chrome = 'card', className }: AnalysisFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/tools/youtube-analyzer/analyze', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ channelInput: normalizedInput, save: false }),
      });
      ...
      upsertClientAnalysis(payload.data);
      router.push(analysisPath(payload.data.id));
    } catch (nextError) {
